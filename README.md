# 关于个人博客
在大学自学PHP的时候，当时用ThinkPHP3.2.3写了一个个人博客，而且还通过个人博客把大学期间自学的内容记录下来。大四实习期间，那段因为工作比较忙，把自己博客的服务器到期的事情都给忘记了，代码虽然有备份，但是数据库没有做备份，自己博客的数据全都丢了。。。

最近工作上遇到的问题和学习的内容，自己又想以个人博客的方式进行记录，因为原来的博客的服务器被回收，干脆就用新技术把个人博客代码重构一下再部署到新的服务器。最初重构打算采用Laravel框架重构，因为PHP算是我的启蒙语言，web开发中，我还是更擅长使用php框架。  
后来想了想，自己已经转战大数据方向，再加上爱折腾的性格，最后决定使用Java web重写自己的博客。

# 开发模式
项目后端采用Java框架spring-boot的最新版本2.1.0进行开发，项目前端采用前端框架React16.4进行开发。项目采用前后端分离的方式，后端接口遵守RestfulAPI。

# 接口设计规范
本项目接口遵守Reast API 接口规范进行开发。有关Rest API接口设计的详细规范如下
